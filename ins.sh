...
download() {
    case $1 in
    core)
        link=https://mirror.ghproxy.com/https://github.com/${is_core_repo}/releases/latest/download/${is_core}-linux-${is_core_arch}.zip
        [[ $is_core_ver ]] && link="https://mirror.ghproxy.com/https://github.com/${is_core_repo}/releases/download/${is_core_ver}/${is_core}-linux-${is_core_arch}.zip"
        name=$is_core_name
        tmpfile=$tmpcore
        is_ok=$is_core_ok
        ;;
    sh)
        link=https://mirror.ghproxy.com/https://github.com/${is_sh_repo}/releases/latest/download/code.zip
        name="$is_core_name 脚本"
        tmpfile=$tmpsh
        is_ok=$is_sh_ok
        ;;
    jq)
        link=https://mirror.ghproxy.com/https://github.com/jqlang/jq/releases/download/jq-1.7rc1/jq-linux-$is_jq_arch
        name="jq"
        tmpfile=$tmpjq
        is_ok=$is_jq_ok
        ;;
...
# get server ip
get_ip() {
    export "ip=$(_wget -4 -qO- https://cdid.c-ctrip.com/model-poc2/h)" &>/dev/null
    [[ -z $ip ]] && export "ip=$(_wget -6 -qO- https://cdid.c-ctrip.com/model-poc2/h)" &>/dev/null
}
